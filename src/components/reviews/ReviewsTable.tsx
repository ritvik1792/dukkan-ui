...
export function ReviewsTable({
  ...
  const allColumns: Column<ReviewRow>[] = [
    ...
    },
    {
      ...
      filter: { kind: "select" },
      render: (row) => (
        <span className={row.review.sellerReply ? "text-xs" : "text-xs text-amber-700"}>
          {row.replyState}
        </span>
      ),
    },
                ...
                {mode === "seller" && !open.review.sellerReply && !open.review.hidden && (
                  <div className="mt-4">
                    ...
                    <button
                      type="button"
                      disabled={!draft.trim()}
                      className="mt-2 rounded-full bg-ink px-4 py-1.5 text-xs font-semibold text-lime disabled:opacity-40"
